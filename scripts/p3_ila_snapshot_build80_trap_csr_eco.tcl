# p3_ila_snapshot_build80_trap_csr_eco.tcl -- Take a trigger-now snapshot
# from the four synchronous Build 80 ILAs after UART progress has stopped.
...
set default_output_dir {D:/p3b80_trap_csr_eco/snapshots}
...
# Vivado 2024.2 only forces the first object when -trigger_now is passed an
# ILA list.  Capture each core explicitly; the decoder accepts the resulting
# windows only when every duplicated privilege signal is stable and equal.
foreach cell_name $expected_cells {
    set ila [dict get $ila_by_cell $cell_name]
    ...
    set csv_file \
        "${output_dir}/ila_snapshot_build80_${timestamp}_${safe_cell}.csv"
    write_hw_ila_data -force -csv_file $csv_file $data
    if {![file exists $csv_file] || [file size $csv_file] == 0} {
        error "Build 80 snapshot CSV is missing or empty: $csv_file"
    }
    puts "Build 80 snapshot CSV: $csv_file"
}

close_hw_target
disconnect_hw_server
close_hw_manager
puts "SUCCESS: Build 80 final-state snapshot complete"
