# p3_build46_axi_translated_ddr.tcl -- Build P3 OpenPiton with the Build 44
# no-stack AXI16550 DDR probe, but translate CPU physical DDR addresses to
# the legal low DDR aperture before the BD AXI NoC.
#
# Usage:
#   vivado -mode batch -source scripts/p3_build46_axi_translated_ddr.tcl -tclargs -jobs 1
#   vivado -mode batch -source scripts/p3_build46_axi_translated_ddr.tcl -tclargs -skip_create -skip_prepare -reuse_synth -jobs 1
#
# By default this build uses D:/p3b46 as the Vivado work directory to avoid
# Windows 260-byte path failures in Versal debug child-IP generation. Override
# with P3_BUILD46_WORK_DIR if needed. Published PDI/LTX files still land under
# the repository's huaprop3_build46_axi_translated_ddr/debug_build directory.

set script_dir [file dirname [info script]]
set repo_dir [file normalize "${script_dir}/.."]
set project_name "huaprop3_build46_axi_translated_ddr"
set output_project_dir [file normalize "${repo_dir}/${project_name}"]
if {[info exists env(P3_BUILD46_WORK_DIR)] && $env(P3_BUILD46_WORK_DIR) ne ""} {
    set project_dir [file normalize $env(P3_BUILD46_WORK_DIR)]
} else {
    set project_dir [file normalize "D:/p3b46"]
}
set output_dir "${output_project_dir}/debug_build"
set create_tcl [file normalize "${script_dir}/p3_create_bd_build46_axi_translated_ddr.tcl"]
set prepare_tcl [file normalize "${script_dir}/p3_prepare_build46_axi_translated_ddr_ila.tcl"]
set bootrom_rebuild_sh [file normalize "${script_dir}/p3_rebuild_build46_axi_translated_ddr.sh"]
set ariane_unread_impl_src [file normalize "${repo_dir}/piton/design/xilinx/huaprop3/unread_vivado_impl.sv"]
set synth_run "synth_1"
set impl_run "impl_1"
set pdi_basename "p3_top_build46_axi_translated_ddr"
set run_create 1
set run_prepare 1
set reuse_synth 0
set jobs 1
...
proc p3_seed_build46_impl_caches {project_dir project_name repo_dir} {
    ...
    foreach param [list synth.maxThreads synth.maxClusterJobsRunCount] {
        ...
        puts "Build 46 setting ${param} from ${old_value} to 1 for implementation child IP generation."
...
proc p3_copy_run_output {run_dir output_dir pdi_basename} {
    ...
    foreach token [list \
        ...
        if {[string first $token $ltx_data] < 0} {
            puts "ERROR: Build 46 LTX missing token: ${token}"
            exit 1
        }
    }
    ...
    puts "Published Build 46 PDI: ${pdi_dst}"
    puts "Published Build 46 LTX: ${ltx_dst}"
}

puts "=========================================="
puts " Build 46: P3 no-stack AXI16550 DDR translated-address probe"
...
if {![file exists $bootrom_rebuild_sh]} {
    puts "ERROR: missing Build 46 bootrom rebuild script: $bootrom_rebuild_sh"
    exit 1
}
puts "Regenerating Build 46 bootrom before Vivado project creation..."
...
if {[catch {exec bash $bootrom_rebuild_exec_path 2>@1} bootrom_rebuild_log]} {
    ...
    puts "ERROR: Build 46 bootrom regeneration failed"
    exit 1
}
puts $bootrom_rebuild_log
...
if {$run_prepare} {
    ...
    puts "Skipping Build 46 prepare step."
}
...
if {[lsearch -exact $defs "P3_SIFIVE_UART"] >= 0 ||
    ...
    puts "ERROR: Build 46 must not enable SiFive UART macros"
    exit 1
}
if {[lsearch -exact $defs "PITONSYS_MEM_ZEROER"] >= 0} {
    puts "ERROR: Build 46 must keep PITONSYS_MEM_ZEROER disabled for the no-stack DDR address-translation probe"
    exit 1
}
...
p3_seed_build46_impl_caches $project_dir $project_name $repo_dir
...
close_project
puts "=========================================="
puts " Build 46 no-stack AXI16550 DDR translated-address probe flow complete"
puts "=========================================="
