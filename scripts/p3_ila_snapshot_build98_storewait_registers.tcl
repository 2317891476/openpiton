# Snapshot Build 98's STORE_WAIT Q/D/CE probes after programming.

set hw_server_url "100.93.77.36"
set hw_server_port "3121"
set xvc_host "202.197.4.99"
set xvc_port "2540"
set default_ltx {C:/p3eco/build98/p3_top_build98_storewait_registers.ltx}
set default_output_dir {C:/p3eco/build98/ila_snapshot}
...
if {![file exists $ltx_file] || [file size $ltx_file] == 0} {
    error "Build 98 LTX missing: $ltx_file"
}
...
set armed [list]
foreach cell $cells {
    if {![dict exists $ila_by_cell $cell]} { error "Build 98 ILA not enumerated: $cell" }
...
foreach ila $armed {
    ...
    set csv "${output_dir}/ila_snapshot_build98_${ts}_${safe}.csv"
    write_hw_ila_data -force -csv_file $csv $data
    if {![file exists $csv] || [file size $csv] == 0} {
        error "Build 98 snapshot CSV missing: $csv"
    }
    puts "Build 98 STORE_WAIT snapshot: $csv"
}
close_hw_target
disconnect_hw_server
close_hw_manager
puts "SUCCESS: Build 98 STORE_WAIT snapshot complete"
