...
/ {
    #address-cells = <2>;
    #size-cells = <2>;
    compatible = "openpiton,build66-companion-bootrom";
...
"${CROSSCOMPILE:-riscv64-unknown-elf-}gcc" -Tlinker.ld bootrom.S \
    -nostdlib -static -Wl,--no-gc-sections -o bootrom.elf
...
if ! awk -v module_name="bootrom" '$1 == "module" && $2 == module_name { found = 1 } END { exit found ? 0 : 1 }' bootrom.sv; then
    echo "ERROR: Build 66 baremetal bootrom.sv does not define module bootrom" >&2
    exit 1
fi
echo "Build 66 companion baremetal bootrom generated: $baremetal_bootrom_dir/bootrom.sv"

bootrom_dir="$repo_dir/piton/design/chipset/rv64_platform/bootrom/linux"
cd "$bootrom_dir"

make clean
make all BOOTROM_MODE=normal PITON_SIFIVE_UART=0 MAX_HARTS=1 UART_FREQ="$CONFIG_SYS_FREQ"

riscv64-unknown-elf-objdump -d bootrom_linux.elf > bootrom_linux_build66.dump
if ! grep -q '<_prog_start>' bootrom_linux_build66.dump; then
    echo "ERROR: Build 66 bootrom is missing _prog_start" >&2
    exit 1
fi
if ! grep -qE '\bsp,' bootrom_linux_build66.dump; then
    echo "ERROR: Build 66 normal bootrom did not set the C stack pointer" >&2
    exit 1
fi
if ! riscv64-unknown-elf-nm bootrom_linux.elf | grep -qE ' main$'; then
    echo "ERROR: Build 66 bootrom is missing main()" >&2
    exit 1
fi
for symbol in init_uart print_uart gpt_find_boot_partition sd_copy; do
    if ! riscv64-unknown-elf-nm bootrom_linux.elf | grep -qE " ${symbol}$"; then
        echo "ERROR: Build 66 bootrom missing expected C symbol: ${symbol}" >&2
        exit 1
    fi
done
if riscv64-unknown-elf-nm bootrom_linux.elf | grep -qE ' startup_asm_uart| startup_asm_uart16550'; then
    echo "ERROR: Build 66 bootrom unexpectedly linked an assembly UART-only mode" >&2
    exit 1
fi
echo "Build 66 normal AXI16550 SPI-SD bootrom generated: $bootrom_dir/bootrom_linux.sv"
