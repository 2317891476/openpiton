# p3_ila_snapshot_build78_store_path_eco.tcl -- Take immediate snapshots from
# the four BD-owned Build 78 ILAs after UART progress has stopped.

set hw_server_url "100.93.77.36"
set hw_server_port "3121"
set xvc_host "202.197.4.99"
set xvc_port "2540"
set default_ltx {D:/p3b78_store_path_eco/p3_top_build78_store_path_eco.ltx}
set default_output_dir {D:/p3b78_store_path_eco/captures}
...
if {![file exists $ltx_file] || [file size $ltx_file] == 0} {
    puts "ERROR: Build 78 LTX is missing or empty: $ltx_file"
    exit 1
}
...
foreach cell_name $expected_cells {
    if {![dict exists $ila_by_cell $cell_name]} {
        puts "ERROR: expected Build 78 ILA was not enumerated: $cell_name"
        exit 1
    }
}
...
foreach cell_name [list \
    ...
    set csv_file \
        "${output_dir}/ila_snapshot_build78_${timestamp}_${safe_cell}.csv"
    write_hw_ila_data -force -csv_file $csv_file $data
    puts "Build 78 snapshot CSV: $csv_file"
}

close_hw_target
disconnect_hw_server
close_hw_manager
puts "SUCCESS: Build 78 immediate ILA snapshots complete"
