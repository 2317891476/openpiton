...
riscv64-unknown-elf-objdump -d bootrom_linux.elf > bootrom_linux_build45.dump
if grep -qE '\bsp\b|0xf000000000' bootrom_linux_build45.dump; then
    echo "ERROR: Build 45 bootrom unexpectedly references stack or SD" >&2
    exit 1
fi
if ! grep -q '<_prog_start>' bootrom_linux_build45.dump; then
    echo "ERROR: Build 45 bootrom is missing _prog_start" >&2
    exit 1
fi
for pattern in 'sb[[:space:]].*,1\(t0\)' \
               ...
               'ld[[:space:]].*,0\(t5\)' \
               'fence'; do
    if ! grep -qE "$pattern" bootrom_linux_build45.dump; then
        echo "ERROR: Build 45 bootrom missing expected UART/DDR access pattern: $pattern" >&2
        exit 1
    fi
done
if riscv64-unknown-elf-nm bootrom_linux.elf | grep -qE ' start_kernel| print_uart| init_uart| sd_init| gpt_find_boot_partition'; then
    echo "ERROR: Build 45 bootrom linked C bootrom symbols" >&2
    exit 1
fi
echo "Build 45 asm UART16550 DDR physical-address probe bootrom generated: $bootrom_dir/bootrom_linux.sv"
