# Capture axis_ila_0 (BD heartbeat/status) + control ILA at stall.
...
# Capture ALL ILAs including axis_ila_0
set cells [list u_bd/openpiton_top_i/axis_ila_0 u_ila_build90 u_bd/openpiton_top_i/axis_ila_1 u_bd/openpiton_top_i/axis_ila_2 u_bd/openpiton_top_i/axis_ila_3]
set armed [list]
foreach cell $cells {
...
foreach ila $armed {
    ...
    set csv "${output_dir}/ila_snapshot_b94full_${ts}_${safe}.csv"
    write_hw_ila_data -force -csv_file $csv $data
    if {![file exists $csv] || [file size $csv] == 0} { error "CSV missing: $csv" }
    puts "Build 94 full CSV: $csv"
}
close_hw_target
disconnect_hw_server
close_hw_manager
puts "SUCCESS: Build 94 full snapshot complete"
