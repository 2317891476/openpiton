#!/usr/bin/env python3
"""Decode Build 48 boot-progress ILA CSV captures."""
...
def decode_uart_bus(path):
    raw = one_probe_value(path, "axis_ila_2 uart")
    decoded = {
        "s_axi_rdata8": (raw >> 56) & 0xff,
        "s_axi_araddr8": (raw >> 48) & 0xff,
        "s_axi_wdata8": (raw >> 40) & 0xff,
        "s_axi_awaddr8": (raw >> 32) & 0xff,
        "core_rdata8": (raw >> 24) & 0xff,
        "core_araddr8": (raw >> 16) & 0xff,
        "core_wdata8": (raw >> 8) & 0xff,
        "core_awaddr8": raw & 0xff,
    }
...
def main():
    ...
    parser.add_argument("debug_build_dir", help="Build 48 debug_build directory")
    args = parser.parse_args()
    try:
        axis0 = find_one(args.debug_build_dir, "ila_capture_build48_*axis_ila_0.csv")
        axis1 = find_one(args.debug_build_dir, "ila_capture_build48_*axis_ila_1.csv")
        axis2 = find_one(args.debug_build_dir, "ila_capture_build48_*axis_ila_2.csv")
        axis3 = find_one(args.debug_build_dir, "ila_capture_build48_*axis_ila_3.csv")
        status = decode_axis0(axis0)
        core = decode_core_bus(axis1)
        uart = decode_uart_bus(axis2)
    ...
    if status["uart_seen"] is None:
        warnings.append("uart_seen16 probe missing")
    else:
        if not ((status["uart_seen"] >> 0) & 1):
            warnings.append("No core-side UART write-address fire observed")
        if not ((status["uart_seen"] >> 3) & 1):
            warnings.append("UART TX-low was not observed by sticky probe")
    ...
    print("PASS: Build 48 ILA decode completed without hard failures")
    return 0
