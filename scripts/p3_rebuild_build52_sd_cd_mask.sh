...
riscv64-unknown-elf-objdump -d bootrom_linux.elf > bootrom_linux_build52.dump
if ! grep -q '<_prog_start>' bootrom_linux_build52.dump; then
    echo "ERROR: Build 52 bootrom is missing _prog_start" >&2
    exit 1
fi
if grep -qE '\bsp,' bootrom_linux_build52.dump; then
    echo "ERROR: Build 52 no-stack SD-CD-mask bootrom unexpectedly touches sp" >&2
    exit 1
fi
if ! riscv64-unknown-elf-nm bootrom_linux.elf | grep -qE ' main$'; then
    echo "ERROR: Build 52 bootrom is missing the assembly main alias" >&2
    exit 1
fi
if riscv64-unknown-elf-nm bootrom_linux.elf | grep -qE ' init_uart$| print_uart$| gpt_find_boot_partition$| sd_copy$'; then
    echo "ERROR: Build 52 no-stack SD-CD-mask bootrom unexpectedly linked C bootrom code" >&2
    exit 1
fi
echo "Build 52 no-stack AXI16550 SD-CD-mask probe bootrom generated: $bootrom_dir/bootrom_linux.sv"
