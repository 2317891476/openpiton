...
make clean
make all BOOTROM_MODE=asm_uart PITON_SIFIVE_UART=1 MAX_HARTS=1 UART_FREQ="$CONFIG_SYS_FREQ"

riscv64-unknown-elf-objdump -d bootrom_linux.elf > bootrom_linux_build42a.dump
if grep -qE '\bsp\b|0x84000000|0x80000000' bootrom_linux_build42a.dump; then
    echo "ERROR: Build 42-A bootrom unexpectedly references stack or DDR" >&2
    exit 1
fi
if ! grep -q '<_prog_start>' bootrom_linux_build42a.dump; then
    echo "ERROR: Build 42-A bootrom is missing _prog_start" >&2
    exit 1
fi
for pattern in 'sw[[:space:]].*,24\(t0\)' \
               'sw[[:space:]].*,8\(t0\)' \
               'sw[[:space:]].*,12\(t0\)' \
               'sw[[:space:]].*,16\(t0\)' \
               'lw[[:space:]].*,0\(t0\)' \
               'sw[[:space:]].*,0\(t0\)'; do
    if ! grep -qE "$pattern" bootrom_linux_build42a.dump; then
        echo "ERROR: Build 42-A bootrom missing expected UART access pattern: $pattern" >&2
        exit 1
    fi
done
if riscv64-unknown-elf-nm bootrom_linux.elf | grep -qE ' start_kernel| print_uart| init_uart'; then
    echo "ERROR: Build 42-A bootrom linked C bootrom symbols" >&2
    exit 1
fi
echo "Build 42-A asm UART bootrom generated: $bootrom_dir/bootrom_linux.sv"
