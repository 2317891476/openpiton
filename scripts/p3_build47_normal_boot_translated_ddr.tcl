# p3_build47_normal_boot_translated_ddr.tcl -- Build P3 OpenPiton with the
# normal C bootrom, original AXI16550 UART, and the Build 46 CPU-physical to
# BD-low DDR address translation.
#
# Usage:
...
#
# By default this build uses D:/p3b47 as the Vivado work directory to avoid
# Windows 260-byte path failures in Versal debug child-IP generation. Override
# with P3_BUILD47_WORK_DIR if needed. Published PDI/LTX files still land under
# the repository's huaprop3_build47_normal_boot_translated_ddr/debug_build
# directory.

set script_dir [file dirname [info script]]
set repo_dir [file normalize "${script_dir}/.."]
set project_name "huaprop3_build47_normal_boot_translated_ddr"
set output_project_dir [file normalize "${repo_dir}/${project_name}"]
if {[info exists env(P3_BUILD47_WORK_DIR)] && $env(P3_BUILD47_WORK_DIR) ne ""} {
    set project_dir [file normalize $env(P3_BUILD47_WORK_DIR)]
} else {
    set project_dir [file normalize "D:/p3b47"]
}
set output_dir "${output_project_dir}/debug_build"
set create_tcl [file normalize "${script_dir}/p3_create_bd_build47_normal_boot_translated_ddr.tcl"]
set prepare_tcl [file normalize "${script_dir}/p3_prepare_build47_normal_boot_ila.tcl"]
set bootrom_rebuild_sh [file normalize "${script_dir}/p3_rebuild_build47_normal_boot.sh"]
set ariane_unread_impl_src [file normalize "${repo_dir}/piton/design/xilinx/huaprop3/unread_vivado_impl.sv"]
set synth_run "synth_1"
set impl_run "impl_1"
set pdi_basename "p3_top_build47_normal_boot_translated_ddr"
set run_create 1
set run_prepare 1
set reuse_synth 0
set jobs 1
...
proc p3_seed_build47_impl_caches {project_dir project_name repo_dir} {
    ...
    foreach param [list synth.maxThreads synth.maxClusterJobsRunCount] {
        if {[catch {get_param $param} old_value] == 0} {
            puts "Build 47 setting ${param} from ${old_value} to 1 for implementation child IP generation."
...
proc p3_copy_run_output {run_dir output_dir pdi_basename} {
    ...
    foreach token [list \
        ...
        if {[string first $token $ltx_data] < 0} {
            puts "ERROR: Build 47 LTX missing token: ${token}"
            exit 1
        }
    }
    ...
    puts "Published Build 47 PDI: ${pdi_dst}"
    puts "Published Build 47 LTX: ${ltx_dst}"
}

puts "=========================================="
puts " Build 47: P3 normal AXI16550 bootrom with translated DDR"
...
if {![file exists $bootrom_rebuild_sh]} {
    puts "ERROR: missing Build 47 bootrom rebuild script: $bootrom_rebuild_sh"
    exit 1
}
puts "Regenerating Build 47 bootrom before Vivado project creation..."
...
if {[catch {exec bash $bootrom_rebuild_exec_path 2>@1} bootrom_rebuild_log]} {
    ...
    puts "ERROR: Build 47 bootrom regeneration failed"
    exit 1
}
puts $bootrom_rebuild_log
...
if {$run_prepare} {
    ...
    puts "Skipping Build 47 prepare step."
}
...
if {[lsearch -exact $defs "P3_SIFIVE_UART"] >= 0 ||
    ...
    puts "ERROR: Build 47 must not enable SiFive UART macros"
    exit 1
}
if {[lsearch -exact $defs "PITONSYS_MEM_ZEROER"] >= 0} {
    puts "ERROR: Build 47 keeps PITONSYS_MEM_ZEROER disabled to isolate normal bootrom traffic"
    exit 1
}
...
p3_seed_build47_impl_caches $project_dir $project_name $repo_dir
...
close_project
puts "=========================================="
puts " Build 47 normal AXI16550 bootrom translated-DDR flow complete"
puts "=========================================="
