# p3_prepare_build47_normal_boot_ila.tcl -- Prepare Build 47 boot-progress ILAs.
# Usage:
#   vivado -mode batch -source scripts/p3_prepare_build47_normal_boot_ila.tcl

set script_dir [file dirname [info script]]
set repo_dir [file normalize "${script_dir}/.."]
if {![info exists P3_PROJECT_NAME] || $P3_PROJECT_NAME eq ""} {
    set P3_PROJECT_NAME "huaprop3_build47_normal_boot_translated_ddr"
}
...
set tmp_dir "Z:/tmp"
set p3_ddr_ila_data_depth 1024
set p3_ddr_ila_input_pipe_stages 0

puts "=========================================="
puts " Build 47 prepare: normal boot DDR/UART ILAs"
...
proc p3_require_tokens {path label tokens} {
    ...
    foreach token $tokens {
        if {[string first $token $data] < 0} {
            puts "ERROR: ${label} is missing expected Build 47 token: ${token}"
            puts "       File: ${path}"
            exit 1
        }
    }
...
proc p3_copy_tmp_rtl {repo_dir tmp_dir} {
    ...
    p3_require_tokens "${tmp_dir}/p3_top.v" "synced P3 top RTL" \
        [list "P3_AXI_DDR_ADDR_TRANSLATE" "P3_BD_DDR_DEBUG_ILA" "P3_BD_BOOT_DEBUG_ILA" \
              "p3_dbg_ddr_seen16_i" "p3_dbg_ddr_bus64_i" "p3_dbg_uart_seen16_i"]
}
...
p3_require_tokens "${repo_dir}/piton/design/chipset/io_ctrl/rtl/uart_top.v" "UART top RTL" \
    [list "PITON_UART16550" "uart_16550"]
...
set_property -dict [list \
    ...
    CONFIG.C_DATA_DEPTH $p3_ddr_ila_data_depth \
    CONFIG.C_EN_STRG_QUAL {0} \
    CONFIG.C_ADV_TRIGGER {false} \
    CONFIG.C_INPUT_PIPE_STAGES $p3_ddr_ila_input_pipe_stages \
...
p3_connect_bd_net_fresh [get_bd_ports p3_dbg_ddr_seen16_i]     [get_bd_pins axis_ila_0/probe2]
...
p3_connect_bd_net_fresh [get_bd_ports p3_dbg_ddr_bus64_i]      [get_bd_pins axis_ila_1/probe0]
...
puts "Generated synth_1 scripts for Build 47."
...
foreach {run_name dcp_file} [list \
    ...
    if {[llength $ila_run] == 0 && [file exists $dcp_file]} {
        puts "Build 47 BD ILA OOC DCP already available without run object: $dcp_file"
        continue
    }
    ...
    puts "Launching Build 47 BD ILA OOC run: $ila_run"
    reset_run $ila_run
    launch_runs $ila_run -jobs 1
    wait_on_run $ila_run
    set ila_status [get_property STATUS $ila_run]
    puts "Build 47 BD ILA OOC run status for ${run_name}: $ila_status"
...
puts "=========================================="
puts " Build 47 prepare complete."
puts "=========================================="
