# p3_ila_capture_build90_rdtime.tcl -- Capture the clean-RTL Build 90
# diagnostic ILA when Linux commits its udelay rdtime instruction.
...
set pc_probe [p3_require_one hw_probe \
...
set csv_file "${output_dir}/ila_capture_build90_rdtime_${timestamp}.csv"
write_hw_ila_data -force -csv_file $csv_file $data
if {![file exists $csv_file] || [file size $csv_file] == 0} {
    error "Build 90 rdtime CSV is missing or empty: $csv_file"
}
puts "Build 90 rdtime CSV: $csv_file"

close_hw_target
disconnect_hw_server
close_hw_manager
puts "SUCCESS: Build 90 rdtime path capture complete"
