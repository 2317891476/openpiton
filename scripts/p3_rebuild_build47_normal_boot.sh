...
riscv64-unknown-elf-objdump -d bootrom_linux.elf > bootrom_linux_build47.dump
if ! grep -q '<_prog_start>' bootrom_linux_build47.dump; then
    echo "ERROR: Build 47 bootrom is missing _prog_start" >&2
    exit 1
fi
if ! grep -qE '\bsp,' bootrom_linux_build47.dump; then
    echo "ERROR: Build 47 normal bootrom did not set the C stack pointer" >&2
    exit 1
fi
if ! riscv64-unknown-elf-nm bootrom_linux.elf | grep -qE ' main$'; then
    echo "ERROR: Build 47 bootrom is missing main()" >&2
    exit 1
fi
for symbol in init_uart print_uart gpt_find_boot_partition; do
    if ! riscv64-unknown-elf-nm bootrom_linux.elf | grep -qE " ${symbol}$"; then
        echo "ERROR: Build 47 bootrom missing expected C symbol: ${symbol}" >&2
        exit 1
    fi
done
if riscv64-unknown-elf-nm bootrom_linux.elf | grep -qE ' startup_asm_uart| startup_asm_uart16550'; then
    echo "ERROR: Build 47 bootrom unexpectedly linked an assembly UART-only mode" >&2
    exit 1
fi
echo "Build 47 normal AXI16550 bootrom generated: $bootrom_dir/bootrom_linux.sv"
