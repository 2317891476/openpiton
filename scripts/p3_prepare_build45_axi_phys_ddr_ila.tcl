# p3_prepare_build45_axi_phys_ddr_ila.tcl -- Prepare Build 45 DDR AXI ILAs.
# Usage:
#   vivado -mode batch -source scripts/p3_prepare_build45_axi_phys_ddr_ila.tcl
...
puts "=========================================="
puts " Build 45 prepare: no-stack DDR AXI ILAs"
...
proc p3_require_tokens {path label tokens} {
    ...
    foreach token $tokens {
        if {[string first $token $data] < 0} {
            puts "ERROR: ${label} is missing expected Build 45 token: ${token}"
            puts "       File: ${path}"
            exit 1
        }
    }
...
proc p3_copy_tmp_rtl {repo_dir tmp_dir} {
    ...
    p3_require_tokens "${tmp_dir}/p3_top.v" "synced P3 top RTL" \
        [list "P3_BD_DDR_DEBUG_ILA" "p3_dbg_ddr_seen16_i" "p3_dbg_ddr_bus64_i"]
}
...
puts "Generated synth_1 scripts for Build 45."
...
foreach {run_name dcp_file} [list \
    ...
    if {[llength $ila_run] == 0 && [file exists $dcp_file]} {
        puts "Build 45 BD ILA OOC DCP already available without run object: $dcp_file"
        continue
    }
    ...
    puts "Launching Build 45 BD ILA OOC run: $ila_run"
    reset_run $ila_run
    launch_runs $ila_run -jobs 1
    wait_on_run $ila_run
    set ila_status [get_property STATUS $ila_run]
    puts "Build 45 BD ILA OOC run status for ${run_name}: $ila_status"
...
puts "=========================================="
puts " Build 45 prepare complete."
puts "=========================================="
