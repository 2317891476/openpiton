#!/usr/bin/env python3
"""Decode Build 52 SD card-detect mask debug ILA CSV captures."""
...
SD_INIT_SEEN_BITS = [
    ...
    "sd_cd_high_seen",
    "sd_cd_low_seen",
    "sd_internal_rst_high_seen",
    "sd_internal_rst_low_seen",
    "init_done_seen",
    "wb_stb_seen",
    "wb_we_seen",
    "wb_ack_seen",
...
def decode_sd_cd_mask_bus(path):
    raw = one_probe_value(path, "axis_ila_2 sd_cd_mask")
    decoded = {
...
def main():
    ...
    parser.add_argument("debug_build_dir", help="Build 52 debug_build directory")
    args = parser.parse_args()
    try:
        axis0 = find_one(args.debug_build_dir, "ila_capture_build52_*axis_ila_0.csv")
        axis1 = find_one(args.debug_build_dir, "ila_capture_build52_*axis_ila_1.csv")
        axis2 = find_one(args.debug_build_dir, "ila_capture_build52_*axis_ila_2.csv")
        axis3 = find_one(args.debug_build_dir, "ila_capture_build52_*axis_ila_3.csv")
        status = decode_axis0(axis0)
        core = decode_core_bus(axis1)
        sd = decode_sd_cd_mask_bus(axis2)
    ...
    if status["sd_seen"] is None:
        warnings.append("sd_cd_mask seen16 probe missing")
    else:
        if (status["sd_seen"] >> 1) & 1 and not ((status["sd_seen"] >> 2) & 1):
            warnings.append("raw sd_cd was only observed high; Build 52 should still release internal SD reset")
    ...
    if sd["flags16"] & (1 << 1) and sd["flags16"] & (1 << 2):
        failures.append("current raw sd_cd=1 and internal SD reset=1; Build 52 card-detect mask is not active")
    elif sd["flags16"] & (1 << 2):
        warnings.append("current internal SD reset is still high")
    elif sd["flags16"] & (1 << 1):
        warnings.append("current raw sd_cd=1, but internal SD reset is released by the Build 52 mask")
    ...
    print("PASS: Build 52 ILA decode completed without hard failures")
    return 0
