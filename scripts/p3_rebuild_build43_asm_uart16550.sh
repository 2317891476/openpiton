...
make clean
make all BOOTROM_MODE=asm_uart16550 PITON_SIFIVE_UART=0 MAX_HARTS=1 UART_FREQ="$CONFIG_SYS_FREQ"

riscv64-unknown-elf-objdump -d bootrom_linux.elf > bootrom_linux_build43.dump
if grep -qE '\bsp\b|0x84000000|0x80000000|0xf000000000' bootrom_linux_build43.dump; then
    echo "ERROR: Build 43 bootrom unexpectedly references stack, DDR, or SD" >&2
    exit 1
fi
if ! grep -q '<_prog_start>' bootrom_linux_build43.dump; then
    echo "ERROR: Build 43 bootrom is missing _prog_start" >&2
    exit 1
fi
for pattern in 'sb[[:space:]].*,1\(t0\)' \
               ...
               'sb[[:space:]].*,4\(t0\)' \
               'lbu[[:space:]].*,5\(t0\)'; do
    if ! grep -qE "$pattern" bootrom_linux_build43.dump; then
        echo "ERROR: Build 43 bootrom missing expected UART16550 access pattern: $pattern" >&2
        exit 1
    fi
done
if riscv64-unknown-elf-nm bootrom_linux.elf | grep -qE ' start_kernel| print_uart| init_uart| sd_init| gpt_find_boot_partition'; then
    echo "ERROR: Build 43 bootrom linked C bootrom symbols" >&2
    exit 1
fi
echo "Build 43 asm UART16550 bootrom generated: $bootrom_dir/bootrom_linux.sv"
