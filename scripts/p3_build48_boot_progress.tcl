# p3_build48_boot_progress.tcl -- Build P3 OpenPiton with the normal C
# bootrom, original AXI16550 UART, translated DDR, and four small BD-owned
# boot-progress ILAs.
#
# Usage:
#   vivado -mode batch -source scripts/p3_build48_boot_progress.tcl -tclargs -jobs 1
#   vivado -mode batch -source scripts/p3_build48_boot_progress.tcl -tclargs -skip_create -skip_prepare -reuse_synth -jobs 1
#
# By default this build uses D:/p3b48 as the Vivado work directory to avoid
# Windows 260-byte path failures in Versal debug child-IP generation. Override
# with P3_BUILD48_WORK_DIR if needed. Published PDI/LTX files still land under
# the repository's huaprop3_build48_boot_progress/debug_build directory.

set script_dir [file dirname [info script]]
set repo_dir [file normalize "${script_dir}/.."]
set project_name "huaprop3_build48_boot_progress"
set output_project_dir [file normalize "${repo_dir}/${project_name}"]
if {[info exists env(P3_BUILD48_WORK_DIR)] && $env(P3_BUILD48_WORK_DIR) ne ""} {
    set project_dir [file normalize $env(P3_BUILD48_WORK_DIR)]
} else {
    set project_dir [file normalize "D:/p3b48"]
}
set output_dir "${output_project_dir}/debug_build"
set create_tcl [file normalize "${script_dir}/p3_create_bd_build48_boot_progress.tcl"]
set prepare_tcl [file normalize "${script_dir}/p3_prepare_build48_boot_progress_ila.tcl"]
set bootrom_rebuild_sh [file normalize "${script_dir}/p3_rebuild_build48_boot_progress.sh"]
set ariane_unread_impl_src [file normalize "${repo_dir}/piton/design/xilinx/huaprop3/unread_vivado_impl.sv"]
set synth_run "synth_1"
set impl_run "impl_1"
set pdi_basename "p3_top_build48_boot_progress"
set run_create 1
set run_prepare 1
set reuse_synth 0
set jobs 1
...
proc p3_seed_build48_impl_caches {project_dir project_name repo_dir} {
    ...
    foreach param [list synth.maxThreads synth.maxClusterJobsRunCount] {
        if {[catch {get_param $param} old_value] == 0} {
            puts "Build 48 setting ${param} from ${old_value} to 1 for implementation child IP generation."
...
proc p3_copy_run_output {run_dir output_dir pdi_basename} {
    ...
    foreach token [list \
        ...
        if {[string first $token $ltx_data] < 0} {
            puts "ERROR: Build 48 LTX missing token: ${token}"
            exit 1
        }
    }
    ...
    puts "Published Build 48 PDI: ${pdi_dst}"
    puts "Published Build 48 LTX: ${ltx_dst}"
}

puts "=========================================="
puts " Build 48: P3 AXI16550 boot-progress debug with translated DDR"
...
if {![file exists $bootrom_rebuild_sh]} {
    puts "ERROR: missing Build 48 bootrom rebuild script: $bootrom_rebuild_sh"
    exit 1
}
puts "Regenerating Build 48 bootrom before Vivado project creation..."
...
if {[catch {exec bash $bootrom_rebuild_exec_path 2>@1} bootrom_rebuild_log]} {
    ...
    puts "ERROR: Build 48 bootrom regeneration failed"
    exit 1
}
puts $bootrom_rebuild_log
...
if {$run_prepare} {
    ...
    puts "Skipping Build 48 prepare step."
}
...
if {[lsearch -exact $defs "P3_SIFIVE_UART"] >= 0 ||
    ...
    puts "ERROR: Build 48 must not enable SiFive UART macros"
    exit 1
}
if {[lsearch -exact $defs "PITONSYS_MEM_ZEROER"] >= 0} {
    puts "ERROR: Build 48 keeps PITONSYS_MEM_ZEROER disabled to isolate normal bootrom traffic"
    exit 1
}
...
p3_seed_build48_impl_caches $project_dir $project_name $repo_dir
...
close_project
puts "=========================================="
puts " Build 48 AXI16550 boot-progress debug flow complete"
puts "=========================================="
