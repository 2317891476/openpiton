# p3_build89_time_csr_boundary_eco.tcl -- Add single-hart P3 CSR_TIME at the
# issue-stage module boundary, without touching internal scoreboard sinks.
#
# CSR 0xc01 returns cycle_q >> 7, matching 30 MHz / 128 = 234375 Hz.  The
# selector also requires a valid CSR commit.  Each functional mux reconnects
# exactly one issue_stage_i input pin; it never rewires segmented-net sinks.

set default_dcp \
    {D:/p3b86_store_queue_eco/p3_top_build86_store_queue_eco_pre_route.dcp}
set default_output_dir {D:/p3b89_time_csr_boundary_eco}
...
set output_base "${output_dir}/p3_top_build89_time_csr_boundary_eco"
...
set resume_from_build89 [string equal -nocase \
...
if {!$resume_from_build89} {
    ...
    set scoreboard "${issue}/i_scoreboard"
    ...
    set sel_low_cell "${cva6}/p3_b89_time_sel_low"
    set sel_high_cell "${cva6}/p3_b89_time_sel_high"
    set addr_sel_cell "${cva6}/p3_b89_time_addr_sel"
    set commit_sel_cell "${cva6}/p3_b89_time_commit_sel"
    set sel_cell "${cva6}/p3_b89_time_sel"
    ...
    # Reconnect exactly the issue-stage module input.  The internal scoreboard
    # input and its memory fanout stay on the issue-stage output net.
    for {set bit 0} {$bit < 64} {incr bit} {
        set sink_pin [p3_require_one pin [get_pins -quiet [list \
            "${issue}/wdata_commit_id\[0\]_27\[${bit}\]"]] \
            "issue-stage commit write-data boundary bit $bit"]
        if {$bit < 57} {
            set cycle_net [p3_q_net \
                "${csr}/cycle_q_reg\[[expr {$bit + 7}]\]"]
            p3_insert_mux_at_pin "${cva6}/p3_b89_time_wdata_${bit}" \
                $cycle_net $time_sel_net $sink_pin
        } else {
            p3_insert_zero_at_pin "${cva6}/p3_b89_time_wdata_${bit}" \
    ...
    set exception_pin [p3_require_one pin [get_pins -quiet [list \
        ...
        {issue-stage CSR exception boundary input}]
    p3_insert_zero_at_pin "${cva6}/p3_b89_time_exception" \
        $time_sel_net $exception_pin

    set eco_cells [get_cells -quiet "${cva6}/p3_b89_time_*"]
    if {[llength $eco_cells] != 70} {
        error "Build 89 created [llength $eco_cells] ECO cells, expected 70"
    }

    puts "Placing 70 Build 89 issue-stage boundary ECO cells"
    place_design -eco -no_timing_driven
    foreach cell $eco_cells {
        if {[get_property LOC $cell] eq ""} {
            error "Build 89 ECO cell remains unplaced: $cell"
        }
    }

    set fh [open $mapping_file w]
    puts $fh "Build 89 single-hart CSR_TIME issue-stage boundary ECO"
    puts $fh "selector: CSR buffer address 0xc01, commit valid, commit FU=CSR"
    puts $fh {data boundary: issue_stage_i/wdata_commit_id[0]_27[63:0] inputs only}
    puts $fh {data[56:0] = cycle_q[63:7]; data[63:57] = 0 when selected}
    puts $fh "exception boundary: issue_stage_i CSR exception input only"
    puts $fh "scoreboard leaf sinks and all segmented-net fanout: unchanged"
    ...
    if {![file exists $mapping_file] || [file size $mapping_file] == 0} {
        error "Build 89 resume requires the existing ECO map: $mapping_file"
    }
    puts "Resuming from exact Build 89 pre-route checkpoint"
}

puts "Routing Build 89 issue-stage boundary TIME CSR ECO"
route_design -eco
...
puts "SUCCESS: Build 89 issue-stage boundary CSR_TIME ECO generated"
