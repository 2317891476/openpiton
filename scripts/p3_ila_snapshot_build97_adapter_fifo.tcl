# Snapshot the Build 97 adapter FIFO / L1.5 ingress probes after programming.

set hw_server_url "100.93.77.36"
set hw_server_port "3121"
set xvc_host "202.197.4.99"
set xvc_port "2540"
set default_ltx {D:/p3b97_adapter_fifo/p3_top_build97_adapter_fifo.ltx}
set default_output_dir {D:/p3b97_adapter_fifo/ila_snapshot}
...
if {![file exists $ltx_file] || [file size $ltx_file] == 0} {
    error "Build 97 LTX missing: $ltx_file"
}
...
set armed [list]
foreach cell $cells {
    if {![dict exists $ila_by_cell $cell]} { error "Build 97 ILA not enumerated: $cell" }
...
foreach ila $armed {
    ...
    set csv "${output_dir}/ila_snapshot_build97_${ts}_${safe}.csv"
    write_hw_ila_data -force -csv_file $csv $data
    if {![file exists $csv] || [file size $csv] == 0} {
        error "Build 97 snapshot CSV missing: $csv"
    }
    puts "Build 97 adapter snapshot: $csv"
}
close_hw_target
disconnect_hw_server
close_hw_manager
puts "SUCCESS: Build 97 adapter FIFO snapshot complete"
