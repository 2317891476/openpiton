# p3_ila_snapshot_build93_load_path.tcl -- Snapshot all Build 93 ILAs at
# the current stable Linux load (PC 0xffffffff8018a7a6 d_set_d_op LW).
set hw_server_url "100.93.77.36"
set hw_server_port "3121"
set xvc_host "202.197.4.99"
set xvc_port "2540"
set default_ltx {D:/p3b93_load_path/p3_top_build93_load_path.ltx}
set default_output_dir {D:/p3b93_load_path/load_path_snapshot}
...
set armed [list]
foreach cell $cells {
    if {![dict exists $ila_by_cell $cell]} { error "Build 93 ILA not enumerated: $cell" }
...
foreach ila $armed {
    ...
    set csv "${output_dir}/ila_snapshot_build93_${ts}_${safe}.csv"
    write_hw_ila_data -force -csv_file $csv $data
    if {![file exists $csv] || [file size $csv] == 0} { error "Build 93 snapshot CSV missing: $csv" }
    puts "Build 93 load-path CSV: $csv"
}
close_hw_target
disconnect_hw_server
close_hw_manager
puts "SUCCESS: Build 93 load-path snapshot complete"
