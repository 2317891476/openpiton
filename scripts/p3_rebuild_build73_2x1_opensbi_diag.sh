...
/ {
    #address-cells = <2>;
    #size-cells = <2>;
    compatible = "openpiton,build73-companion-bootrom";
...
if ! awk -v module_name="bootrom" '$1 == "module" && $2 == module_name { found = 1 } END { exit found ? 0 : 1 }' bootrom.sv; then
    echo "ERROR: Build 73 baremetal bootrom.sv does not define module bootrom" >&2
    exit 1
fi
echo "Build 73 companion baremetal bootrom generated: $baremetal_bootrom_dir/bootrom.sv"
...
riscv64-unknown-elf-objdump -d bootrom_linux.elf > bootrom_linux_build73_2x1_opensbi_diag.dump
if ! grep -q '<_prog_start>' bootrom_linux_build73_2x1_opensbi_diag.dump; then
    echo "ERROR: Build 73 bootrom is missing _prog_start" >&2
    exit 1
fi
if ! grep -qE '\bsp,' bootrom_linux_build73_2x1_opensbi_diag.dump; then
    echo "ERROR: Build 73 bootrom did not set the C stack pointer" >&2
    exit 1
fi
if ! riscv64-unknown-elf-nm bootrom_linux.elf | grep -qE ' main$'; then
    echo "ERROR: Build 73 bootrom is missing main()" >&2
    exit 1
fi
for symbol in init_uart print_uart init_sd sd_copy; do
    if ! riscv64-unknown-elf-nm bootrom_linux.elf | grep -qE " ${symbol}$"; then
        echo "ERROR: Build 73 bootrom missing expected C symbol: ${symbol}" >&2
        exit 1
    fi
done
if ! riscv64-unknown-elf-strings bootrom_linux.elf | grep -q 'B69 DDR OK'; then
    echo "ERROR: Build 73 bootrom is missing the DDR diagnostic string" >&2
    exit 1
fi
if ! riscv64-unknown-elf-strings bootrom_linux.elf | grep -q 'B69 releasing harts and jumping OpenSBI'; then
    echo "ERROR: Build 73 bootrom is missing the OpenSBI handoff diagnostic string" >&2
    exit 1
fi
if riscv64-unknown-elf-nm bootrom_linux.elf | grep -qE ' gpt_find_boot_partition$|startup_asm_uart|startup_asm_uart16550'; then
    echo "ERROR: Build 73 bootrom linked an unexpected BBL or assembly UART-only path" >&2
    exit 1
fi
if ! awk -v module_name="bootrom_linux" '$1 == "module" && $2 == module_name { found = 1 } END { exit found ? 0 : 1 }' bootrom_linux.sv; then
    echo "ERROR: Build 73 bootrom_linux.sv does not define module bootrom_linux" >&2
    exit 1
fi

echo "Build 73 2x1 OpenSBI diagnostic bundle bootrom generated: $bootrom_dir/bootrom_linux.sv"
