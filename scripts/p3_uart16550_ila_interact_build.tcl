# p3_uart16550_ila_interact_build.tcl -- Standalone P3 axi_uart16550 interaction build.
# Usage:
#   vivado -mode batch -source scripts/p3_uart16550_ila_interact_build.tcl -tclargs -jobs 8

set script_dir [file dirname [info script]]
set repo_dir [file normalize "${script_dir}/.."]
set project_name "p3_uart16550_ila_interact"
set output_project_dir [file normalize "${repo_dir}/huaprop3_uart16550_ila_interact"]
...
set bd_name "p3_uart16550_dbg_bd"
set jobs 8
...
if {[info exists env(P3_UART16550_WORK_DIR)]} {
    set project_dir [file normalize $env(P3_UART16550_WORK_DIR)]
} else {
    # Keep this path short. Versal debug hub generation creates nested AXI NoC
    # child projects during opt_design, and Windows Vivado still rejects paths
    # longer than 260 bytes in that flow.
    set project_dir "C:/p3u16550"
}
...
puts "=========================================="
puts " P3 standalone AXI UART16550 interaction build"
...
foreach required [list \
    "${src_dir}/p3_uart16550_ila_interact_top.v" \
    "${src_dir}/p3_uart16550_ila_interact.xdc" \
] {
...
add_files -norecurse "${src_dir}/p3_uart16550_ila_interact_top.v"
add_files -fileset constrs_1 -norecurse "${src_dir}/p3_uart16550_ila_interact.xdc"
...
set_property -dict [list \
    ...
    CONFIG.CLKOUT_PORT {uart_clk,clk_out2,clk_out3,clk_out4,clk_out5,clk_out6,clk_out7} \
...
connect_bd_net [get_bd_pins clk_wizard_0/uart_clk] [get_bd_pins proc_sys_reset_0/slowest_sync_clk]
connect_bd_net [get_bd_pins const_locked/dout] [get_bd_pins proc_sys_reset_0/dcm_locked]
connect_bd_net [get_bd_pins clk_wizard_0/uart_clk] [get_bd_pins axis_ila_0/clk]

create_bd_port -dir O uart_clk_o
create_bd_port -dir O -from 0 -to 0 uart_aresetn_o
create_bd_port -dir I -from 15 -to 0 p3_uart_status_i
create_bd_port -dir I -from 63 -to 0 p3_uart_txn_i
create_bd_port -dir I -from 31 -to 0 p3_uart_counts_i
create_bd_port -dir I -from 0 -to 0 p3_uart_rx_level_i
create_bd_port -dir I -from 0 -to 0 p3_uart_rx_fall_i

connect_bd_net [get_bd_pins clk_wizard_0/uart_clk] [get_bd_ports uart_clk_o]
connect_bd_net [get_bd_pins proc_sys_reset_0/peripheral_aresetn] [get_bd_ports uart_aresetn_o]
connect_bd_net [get_bd_ports p3_uart_status_i] [get_bd_pins axis_ila_0/probe0]
connect_bd_net [get_bd_ports p3_uart_txn_i] [get_bd_pins axis_ila_0/probe1]
connect_bd_net [get_bd_ports p3_uart_counts_i] [get_bd_pins axis_ila_0/probe2]
connect_bd_net [get_bd_ports p3_uart_rx_level_i] [get_bd_pins axis_ila_0/probe3]
connect_bd_net [get_bd_ports p3_uart_rx_fall_i] [get_bd_pins axis_ila_0/probe4]
...
set_property top p3_uart16550_ila_interact_top [current_fileset]
...
set pdi_dst "${output_dir}/p3_uart16550_ila_interact.pdi"
set ltx_dst "${output_dir}/p3_uart16550_ila_interact.ltx"
...
close_project
puts "=========================================="
puts " P3 standalone AXI UART16550 interaction build complete"
puts "=========================================="
