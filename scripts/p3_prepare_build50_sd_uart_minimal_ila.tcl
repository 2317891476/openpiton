# p3_prepare_build50_sd_uart_minimal_ila.tcl -- Prepare Build 50 SD-UART minimal ILAs.
# Usage:
#   vivado -mode batch -source scripts/p3_prepare_build50_sd_uart_minimal_ila.tcl
...
puts "=========================================="
puts " Build 50 prepare: AXI16550 SD-UART minimal ILAs"
...
proc p3_require_tokens {path label tokens} {
    ...
    foreach token $tokens {
        if {[string first $token $data] < 0} {
            puts "ERROR: ${label} is missing expected Build 50 token: ${token}"
            puts "       File: ${path}"
            exit 1
        }
    }
...
puts "Generated synth_1 scripts for Build 50."
...
foreach {run_name dcp_file} [list \
    ...
    if {[llength $ila_run] == 0 && [file exists $dcp_file]} {
        puts "Build 50 BD ILA OOC DCP already available without run object: $dcp_file"
        continue
    }
    ...
    puts "Launching Build 50 BD ILA OOC run: $ila_run"
    reset_run $ila_run
    launch_runs $ila_run -jobs 1
    wait_on_run $ila_run
    set ila_status [get_property STATUS $ila_run]
    puts "Build 50 BD ILA OOC run status for ${run_name}: $ila_status"
...
puts "=========================================="
puts " Build 50 prepare complete."
puts "=========================================="
