# p3_prepare_build48_boot_progress_ila.tcl -- Prepare Build 48 boot-progress ILAs.
# Usage:
#   vivado -mode batch -source scripts/p3_prepare_build48_boot_progress_ila.tcl

set script_dir [file dirname [info script]]
set repo_dir [file normalize "${script_dir}/.."]
if {![info exists P3_PROJECT_NAME] || $P3_PROJECT_NAME eq ""} {
    set P3_PROJECT_NAME "huaprop3_build48_boot_progress"
}
...
puts "=========================================="
puts " Build 48 prepare: AXI16550 boot-progress ILAs"
...
proc p3_require_tokens {path label tokens} {
    ...
    foreach token $tokens {
        if {[string first $token $data] < 0} {
            puts "ERROR: ${label} is missing expected Build 48 token: ${token}"
            puts "       File: ${path}"
            exit 1
        }
    }
...
puts "Generated synth_1 scripts for Build 48."
...
foreach {run_name dcp_file} [list \
    ...
    if {[llength $ila_run] == 0 && [file exists $dcp_file]} {
        puts "Build 48 BD ILA OOC DCP already available without run object: $dcp_file"
        continue
    }
    ...
    puts "Launching Build 48 BD ILA OOC run: $ila_run"
    reset_run $ila_run
    launch_runs $ila_run -jobs 1
    wait_on_run $ila_run
    set ila_status [get_property STATUS $ila_run]
    puts "Build 48 BD ILA OOC run status for ${run_name}: $ila_status"
...
puts "=========================================="
puts " Build 48 prepare complete."
puts "=========================================="
