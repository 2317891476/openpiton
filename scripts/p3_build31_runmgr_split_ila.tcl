# p3_build31_runmgr_split_ila.tcl -- Build 31 using project run-manager BD ILAs.
# Usage:
#   vivado -mode batch -source scripts/p3_build31_runmgr_split_ila.tcl
#   vivado -mode batch -source scripts/p3_build31_runmgr_split_ila.tcl -tclargs -skip_prepare
#   vivado -mode batch -source scripts/p3_build31_runmgr_split_ila.tcl -tclargs -jobs 2
#
# Build 31 intentionally avoids direct read_checkpoint -cell stitching for
# BD-owned axis_ila instances. Builds 29/30 routed successfully, but the direct
# flow lost Chipscope debug-core metadata before write_debug_probes. Running the
# normal project synthesis/implementation flow lets IP Integrator own the BD
# debug IP, AXI debug hub, and PMC_AXI_NOC0 address metadata end to end.
...
set prepare_tcl [file normalize "${script_dir}/p3_prepare_build31_runmgr_split_ila.tcl"]
set synth_run "synth_1"
set impl_run "impl_31_runmgr_split_ila"
set pdi_basename "p3_top_build31_runmgr_split_ila"
set run_prepare 1
set jobs 1
...
for {set i 0} {$i < [llength $argv]} {incr i} {
    ...
    switch -- $arg {
        ...
        default {
            ...
            puts "Usage: vivado -mode batch -source scripts/p3_build31_runmgr_split_ila.tcl ?-tclargs -skip_prepare? ?-jobs N?"
            exit 1
        }
    }
}
...
proc p3_copy_checked_run_output {run_dir output_dir pdi_basename} {
    ...
    if {[string first "0x000003FFC0000000" $ltx_data] < 0 ||
        ...
        puts "ERROR: LTX is missing the expected Build 31 debug hub path or split ILA cells."
    ...
    puts "Published Build 31 PDI: ${pdi_dst}"
    puts "Published Build 31 LTX: ${ltx_dst}"
}

puts "=========================================="
puts " Build 31: run-manager split BD-owned ILAs"
...
if {$run_prepare} {
    ...
    puts "Skipping Build 31 BD preparation step."
}
...
puts "=========================================="
puts " Build 31 run-manager flow complete"
puts "=========================================="
