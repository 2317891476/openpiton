# Build 98 -- diagnostic-only incremental STORE_WAIT Q/D/CE probe ECO.
#
# Reuses the Build 97 routed DCP, changes only existing ILA probe loads, then
...
set default_output_dir {C:/p3eco/build98}
set output_stem {p3_top_build98_storewait_registers}
set tag_prefix {p3_build98}
...
if {![file exists $input_dcp] || [file size $input_dcp] == 0} {
    error "Build 97 routed DCP is missing or empty: $input_dcp"
}
...
puts "Opening Build 97 routed checkpoint: $input_dcp"
...
if {!$resume} {
    ...
    # ILA1 compares the missunit state flops' current, next, and enable
    # values with the FIFO count flops' current, next, and enable values.
    set ila1_base [concat $pc16 [list \
        $src_req $src_ack $no_store_pending \
        $dc_req $dc_req_q $dc_resp_ack \
        $mu_req $mu_req_q $mu_resp_ack] $state_q $state_d $state_ce \
    ...
    reconnect "u_bd/openpiton_top_i/axis_ila_1/probe0" $ila1 "${tag_prefix}_registers"

    # ILA2 retains the lower L1.5 pipeline boundary beside the FIFO's Q/D/CE.
    ...
    # ILA3 is an independent state/enable confirmation window.
    ...
    reconnect "u_bd/openpiton_top_i/axis_ila_3/probe0" $ila3 "${tag_prefix}_confirm"

    set fh [open $probe_map w]
    puts $fh {Build 98 probe map (bit 0 first, 64 bits per ILA)}
    puts $fh "source=$input_dcp"
    puts $fh {ILA1 [15:0]=commit_PC_low [16:24]=source/D-cache/missunit AMO [25:27]=missunit_Q [28:30]=missunit_D [31:33]=missunit_CE [34:35]=FIFO_count_Q [36:37]=FIFO_count_D [38:39]=FIFO_count_CE [40:41]=FIFO_pointer_Q [42:43]=FIFO_pointer_D [44:47]=FIFO_slot_rtype}
    puts $fh {ILA2 [15:0]=commit_PC_low [16:21]=FIFO_count_Q/D/CE [22:25]=FIFO_pointer_Q/D [26:34]=missunit_Q/D/CE [35:42]=L1.5 val/NoC1 credit/ack/router [43:50]=NoC1 command FIFO valid [51:53]=NoC2 valid}
    puts $fh {ILA3 [15:0]=commit_PC_low [16:21]=source/missunit AMO [22:24]=missunit_Q [25:27]=missunit_D [28:30]=missunit_CE [31:36]=FIFO_count_Q/D/CE [37:40]=FIFO_pointer_Q/D [41:44]=FIFO_slot_rtype}
    ...
    puts "Resuming Build 98 from pre-route checkpoint"
}

puts "ECO-routing Build 98 diagnostic probe loads"
route_design -eco
...
puts "SUCCESS: Build 98 incremental STORE_WAIT register diagnostic generated"
puts "  PDI: $pdi_file"
puts "  LTX: $ltx_file"
puts "  DCP: $routed_dcp"
puts "  map: $probe_map"
