# p3_prepare_build44_ddr_ila.tcl -- Prepare Build 44 DDR AXI ILAs.
# Usage:
#   vivado -mode batch -source scripts/p3_prepare_build44_ddr_ila.tcl
...
puts "=========================================="
puts " Build 44 prepare: no-stack DDR AXI ILAs"
...
proc p3_require_tokens {path label tokens} {
    ...
    foreach token $tokens {
        if {[string first $token $data] < 0} {
            puts "ERROR: ${label} is missing expected Build 44 token: ${token}"
            puts "       File: ${path}"
            exit 1
        }
    }
...
puts "Generated synth_1 scripts for Build 44."
...
foreach {run_name dcp_file} [list \
    ...
    if {[llength $ila_run] == 0 && [file exists $dcp_file]} {
        puts "Build 44 BD ILA OOC DCP already available without run object: $dcp_file"
        continue
    }
    ...
    puts "Launching Build 44 BD ILA OOC run: $ila_run"
    reset_run $ila_run
    launch_runs $ila_run -jobs 1
    wait_on_run $ila_run
    set ila_status [get_property STATUS $ila_run]
    puts "Build 44 BD ILA OOC run status for ${run_name}: $ila_status"
...
puts "=========================================="
puts " Build 44 prepare complete."
puts "=========================================="
