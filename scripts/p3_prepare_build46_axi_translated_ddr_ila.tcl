# p3_prepare_build46_axi_translated_ddr_ila.tcl -- Prepare Build 46 DDR AXI ILAs.
# Usage:
#   vivado -mode batch -source scripts/p3_prepare_build46_axi_translated_ddr_ila.tcl

set script_dir [file dirname [info script]]
set repo_dir [file normalize "${script_dir}/.."]
if {![info exists P3_PROJECT_NAME] || $P3_PROJECT_NAME eq ""} {
    set P3_PROJECT_NAME "huaprop3_build46_axi_translated_ddr"
}
...
puts "=========================================="
puts " Build 46 prepare: no-stack DDR AXI ILAs"
...
proc p3_require_tokens {path label tokens} {
    ...
    foreach token $tokens {
        if {[string first $token $data] < 0} {
            puts "ERROR: ${label} is missing expected Build 46 token: ${token}"
            puts "       File: ${path}"
            exit 1
        }
    }
...
proc p3_copy_tmp_rtl {repo_dir tmp_dir} {
    ...
    p3_require_tokens "${tmp_dir}/p3_top.v" "synced P3 top RTL" \
        [list "P3_AXI_DDR_ADDR_TRANSLATE" "P3_BD_DDR_DEBUG_ILA" "p3_dbg_ddr_seen16_i" "p3_dbg_ddr_bus64_i"]
}
...
puts "Generated synth_1 scripts for Build 46."
...
foreach {run_name dcp_file} [list \
    ...
    if {[llength $ila_run] == 0 && [file exists $dcp_file]} {
        puts "Build 46 BD ILA OOC DCP already available without run object: $dcp_file"
        continue
    }
    ...
    puts "Launching Build 46 BD ILA OOC run: $ila_run"
    reset_run $ila_run
    launch_runs $ila_run -jobs 1
    wait_on_run $ila_run
    set ila_status [get_property STATUS $ila_run]
    puts "Build 46 BD ILA OOC run status for ${run_name}: $ila_status"
...
puts "=========================================="
puts " Build 46 prepare complete."
puts "=========================================="
