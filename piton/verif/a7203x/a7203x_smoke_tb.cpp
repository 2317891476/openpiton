...
#include "Va7203x_smoke_top.h"
#include "verilated.h"

static vluint64_t sim_time = 0;

static void tick(Va7203x_smoke_top* top) {
...
static void run_cycles(Va7203x_smoke_top* top, int cycles) {
...
int main(int argc, char** argv) {
    ...
    const uint8_t initial_leds = top.leds;
    bool leds_changed_during_reset = false;
    for (int i = 0; i < 128; i++) {
        tick(&top);
        if (top.leds != initial_leds) {
            leds_changed_during_reset = true;
        }
    }

    if (!leds_changed_during_reset) {
        std::cerr << "TEST: FAIL leds did not toggle while reset was asserted\n";
        return 1;
    }
    ...
    const std::string expected = "AX7203 SMOKE\r\n";
    std::string received;
    ...
    std::cout << "TEST: PASS AX7203 smoke top\n";
    std::cout << "TEST: uart_banner=AX7203 SMOKE\\r\\n\n";
