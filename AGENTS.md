...
Future Build 68 remote runs default to `JOBS=16`. Override with `JOBS=<N> scripts/p3_remote_vivado_64core.sh` only for an intentional experiment. The active 2026-06-12 run was launched with `-jobs 8`; Vivado used up to 7 synthesis processes and up to 8 CPUs for place/route. The offline Ubuntu host has 384 logical CPUs and 1.5 TiB RAM, so 16 is the conservative default for the next rerun. The shared Build 52 wrapper sets `general.maxThreads` and `synth.maxThreads` for the top-level phases, while preserving the existing child-IP serialization workaround after synthesis.
