...
Future Build 68 remote runs default to `JOBS=16`. Override with `JOBS=<N> scripts/p3_remote_vivado_64core.sh` only for an intentional experiment. The active 2026-06-12 run was launched with `-jobs 8`; Vivado used up to 7 synthesis processes and up to 8 CPUs for place/route. The offline Ubuntu host has 384 logical CPUs, 192 physical cores, and 1.5 TiB RAM. `JOBS=32` is the current practical upper bound for the next controlled experiment; do not use 64+ as the default until a 16/32 comparison is collected and file-descriptor pressure is checked (`ulimit -n` was 1024). The shared Build 52 wrapper sets `general.maxThreads` and `synth.maxThreads` for the top-level phases, while preserving the existing child-IP serialization workaround after synthesis.
