...
The next Build 68 rerun cleared the Ariane debug-port error and failed later in `riscv_peripherals`: Vivado reported `Synth 8-439 module 'bootrom' not found` at `riscv_peripherals.sv:381`. The concrete root cause is that `riscv_peripherals.sv` instantiates both `bootrom` and `bootrom_linux` and selects between them with `ariane_boot_sel_i`; Build 68's clean remote archive regenerated only `bootrom/linux/bootrom_linux.sv`, while the required baremetal `bootrom/baremetal/bootrom.sv` existed only as an old untracked local generated file. `scripts/p3_rebuild_build68_8x8_opensbi_linux.sh` now regenerates and module-checks both ROMs before Vivado project creation, so a self-contained Build 68 snapshot should contain both generated sources. The companion baremetal ROM is generated directly from the tracked `bootrom/baremetal/rv64_platform.dts`, not through `riscvlib.py`, because the remote archive intentionally has no `.git` metadata and Vivado's bundled shell environment exposed an `awk -e` incompatibility in the riscvlib version-string pipeline. Treat the old failure as a bootrom generation/snapshot bug before synthesis resource reporting, not as a 64-core resource, BRAM, placement, routing, congestion, or timing result.
