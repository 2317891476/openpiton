...
Build 68 remote runs default to `JOBS=32` after the 2026-06-12 host capacity check. The earlier active run used `-jobs 8`; Vivado reported main synthesis using up to 7 processes and place/route using up to 8 CPUs. The offline Ubuntu host has 384 logical CPUs, 192 physical cores, and 1.5 TiB RAM; the 8x8 run peaked near 105 GiB RSS during post-route reporting, leaving about 1.3 TiB RAM available. Do not jump directly to 64+ by default: Vivado place/route scaling is not linear, and the current shell file-descriptor limit is `ulimit -n=1024`. The shared Build 52 wrapper also sets `general.maxThreads` and `synth.maxThreads` to the chosen job count before top synthesis and place/route; its implementation child-IP cache workaround still serializes generated child-IP synthesis after the top synthesis stage.
...
The next Build 68 rerun cleared the Ariane debug-port error and failed later in `riscv_peripherals`: Vivado reported `Synth 8-439 module 'bootrom' not found` at `riscv_peripherals.sv:381`. The concrete root cause is that `riscv_peripherals.sv` instantiates both `bootrom` and `bootrom_linux` and selects between them with `ariane_boot_sel_i`; Build 68's clean remote archive regenerated only `bootrom/linux/bootrom_linux.sv`, while the required baremetal `bootrom/baremetal/bootrom.sv` existed only as an old untracked local generated file. `scripts/p3_rebuild_build68_8x8_opensbi_linux.sh` now regenerates and module-checks both ROMs before Vivado project creation, so a self-contained Build 68 snapshot should contain both generated sources. The companion baremetal ROM uses an inline minimal DTS, not `riscvlib.py` or the `bootrom/baremetal/rv64_platform.dts` symlink, because the remote archive intentionally has no `.git` metadata and the symlink target `bootrom/rv64_platform.dts` is an ignored generated file. Treat the old failure as a bootrom generation/snapshot bug before synthesis resource reporting, not as a 64-core resource, BRAM, placement, routing, congestion, or timing result.

The 2026-06-12 remote Build 68 run completed through `write_device_image` and published PDI/LTX artifacts under `/home/cs/openpiton/huaprop3_build68_8x8_opensbi_linux/debug_build/`. The PDI was 237,439,824 bytes with SHA256 `c93d558c24080631611ce211fcb6df391730d8d04d943a1d35bc7f960612a379` in the run directory; the LTX was 199,395 bytes with SHA256 `5c945b15727491d5cd6a8cc7391cd383abc1d4e1336a298f847bf628b8a1eac2`. This is a complete VP1902 Vivado build result, not a board-boot proof. Do not claim 64-core Linux success until the generated PDI/LTX are programmed on P3 Pro and UART logs prove OpenSBI, Linux, `SMP: Total of 64 processors activated`, a shell, and a 64-CPU runtime check.
...
Restarting the board-side hw_server did not fix this path: a second Windows-Vivado retry remained in `program_hw_devices` for 34 minutes 58 seconds and again required cancellation. The next recovery removes the long control path by copying the verified PDI/LTX to `illya@100.93.77.36` and running that host's Vivado Lab against its local hw_server plus the same XVC endpoint. Do not regenerate the FPGA image for these stalls; they have not crossed the configuration gate.

The board-local retry on 2026-06-14 proved that the network transport and physical JTAG chain can remain healthy while the Versal configuration/debug state is unusable. Raw XVC scans returned VP1902 IDCODE `0x04c40093` and ARM DAP IDCODE `0x6ba00477`, but detailed `hw_server` tracing ended with `No core debug access` and `dpc-jtag: link down`; Vivado Lab consequently reported `No devices detected`, and XSDB exposed no target that could issue a system reset. Treat this combination as a partial-configuration recovery gate after an interrupted PDI download. Perform a complete board power cycle and retry the same hash-verified PDI before changing RTL or starting another build. UART output collected before a new `DONE bit: HIGH` result is non-actionable.
...
Hardware result: Build 65 completed from `D:/p3b65`, generated `p3_top_build65_spi_sd_block_debug.pdi/.ltx`, and programmed successfully with `DONE bit: HIGH`. The LTX refreshed the AXI debug hub at `0x3ffc0000000`, found all four `axis_ila_*` cores, and exported CSV. Route status had 151,476 fully routed nets and 0 routing errors; timing met constraints with `WNS=17.295 ns`, `WHS=0.016 ns`, and no setup, hold, or pulse-width failing endpoints. The decoder passed: AXI AR, SD request launch, transaction-manager request, block transaction type/control writes, status done, RX-byte read/cache write, transaction DONE, bridge response, `SEND_RESP`, AXI R response, and nonzero data were all observed, while the read-error sticky bit stayed clear. Treat the normal SPI SD transport as working through CPU-facing AXI response; further no-output debug should now inspect bootrom interpretation of returned sectors, SD image/GPT/BBL layout, copy/cache behavior, exception handling, and later UART logging.
...
Self-contained rebuild rule: Build 66 and scaling successors set `P3_SELF_CONTAINED_SOURCES=1`. During project creation, `scripts/p3_create_bd.tcl` copies RTL, headers, XDC constraints, `p3_top.v`, `openpiton_wrapper.v`, and the Vivado unread shim into `<workdir>/source_snapshot/`, then adds Vivado files from that snapshot. Include directories are copied recursively with relative subdirectories preserved, because Ariane/CVA6 resolves files such as `register_interface/assign.svh` and `register_interface/typedef.svh` through nested `include` paths. The Build 52-derived runner fails early if the `.xpr`, fileset, or include dirs reference live repository sources under `piton/`, stale `Z:/tmp` mirrors, or old `D:/p3b*` workspaces. Generated BD wrappers and IP output products stay under the Vivado project `.gen/.srcs/.cache` directories.
...
Implementation result: on 2026-06-05, Build 67 completed from a clean repository-local `p3b67_2x1/source_snapshot/` workspace after fixing the tile PyHP define mismatch. Vivado 2024.2.2 completed synthesis, implementation, route, and `write_device_image` with 0 errors. Published artifacts are `huaprop3_build67_2x1_baseline/debug_build/p3_top_build67_2x1_normal_spi_sd_boot.pdi` with SHA256 `f979264a5e43e5f1e90061590da79ed0d54c080e7aac384762219f993c978ed0` and matching `.ltx` with SHA256 `7b9c348e4d2695dfd90f700aabc8e4eee66288d07dd7a4c81ea636e33d2aeaaf`. Route status reported 258,405 fully routed nets and 0 routing errors; timing met constraints with `WNS=16.312 ns`, `WHS=0.005 ns`, and no setup, hold, or pulse-width failing endpoints. Hardware programming, debug-hub refresh, UART boot, and Linux SMP validation are the next gate before promoting Build 67 beyond candidate status.

Hardware result: the first Build 67 programming run on 2026-06-05 passed the PDI/LTX/debug-hub gate. Vivado reported `Successfully programmed PDI`, `DONE bit: HIGH`, debug hub setup at `0x3ffc0000000`, and four ILAs. UART log `~/p3_uart_logs/ttyUSB0_20260605_175423_b67.log` reached the normal bootrom, SPI-mode SD initialization, GPT payload discovery, full 65,536-block DDR copy, matching DDR/SD payload readback, and `bbl loader`. The board then printed a BBL DTB containing only `cpu@0`; the local `huaprop3_linux_xsbench_2x1.img` contains `cpu@1`, so this run used the stale single-hart SD card payload rather than the 2x1 image. Before rerunning SMP validation, write `build/huaprop3/sd_images/huaprop3_linux_xsbench_2x1.img` to the SD card on the remote Ubuntu reader and verify the card is a nonzero-size removable `/dev/sdX`.

Linux handoff discriminator images: after the 2026-06-08 retest stopped immediately after the two-hart BBL DTB dump, use `scripts/p3_make_build67_linux_handoff_images.sh` to regenerate two SD-image-only variants without rerunning Vivado. `build/huaprop3/sd_images/huaprop3_linux_xsbench_2x1_nosmp.img` has SHA256 `714e07231ebb85c83bb9c89e5233fd801182a6523ce311530e03238f5e31241d`, disables `cpu@1`, and adds `maxcpus=1 nosmp`. `build/huaprop3/sd_images/huaprop3_linux_xsbench_2x1_bbl_markers.img` has SHA256 `048904f50e8ed101f4b595de7cbfa2936e6924e460e57113165454832be54911`, keeps both harts visible, suppresses the long BBL DTB dump, and embeds `B67M` marker prints around BBL entry selection and `mret`. The marker check in the script uses direct binary `grep -a -q` so `set -o pipefail` cannot misreport a successful `grep -q` match as a failure after an upstream SIGPIPE.

Nosmp hardware result: writing `huaprop3_linux_xsbench_2x1_nosmp.img` to `/dev/sdc` and reprogramming the unchanged Build 67 PDI/LTX still reached BBL but not Linux. UART log `~/p3_uart_logs/ttyUSB0_20260608_225030_b67_nosmp_retry.log` completed the bootrom copy and printed a BBL DTB with `maxcpus=1 nosmp` in bootargs and `cpu@1 status = "masked"`. Because no `Linux version` appeared even with hart1 masked, the next software-only test should write the `bbl_markers` image and inspect whether the `B67M` prints reach entry selection and `mret`.

BBL marker hardware result: writing `huaprop3_linux_xsbench_2x1_bbl_markers.img` to `/dev/sdc` and reprogramming the unchanged Build 67 PDI/LTX proved that BBL reaches the Linux handoff. UART log `~/p3_uart_logs/ttyUSB0_20260609_093656_b67_bbl_markers.log` printed `B67M boot_loader dtb_in=0x0000000080006708 dtb_out=0x0000000081000000 disabled=0x0000000000000000` and `B67M mret hart_arg=0x0000000000000000 entry=0x0000000080200000 dtb=0x0000000081000000`. A full-log review also found `Linux version 5.1.0-rc7` in that run, and the normal 2x1 retest log `~/p3_uart_logs/ttyUSB0_20260608_195222_b67_retest.log` reaches `Run /bin/sh as init process`. Keep using the same Build 67 PDI/LTX for the next validation; the immediate task is a long capture plus throttled UART shell interaction, not another Vivado build.

Keep-divisor hardware result: writing `huaprop3_linux_xsbench_2x1_keep_divisor.img` to `/dev/sdc` and reprogramming the unchanged Build 67 PDI/LTX moved the active stop forward without any Vivado rebuild. The image SHA256 is `2fd755442425b508c787ff1b1e601f56dc90a52ea9b49215788e1a4b35f3a980`; the remote SD readback matched. Programming reported `DONE bit: HIGH`, debug hub `0x3ffc0000000`, and four ILAs. UART logs `~/p3_uart_logs/ttyUSB0_20260610_173702.log` plus `~/p3_uart_logs/ttyUSB0_20260610_174508.log` show the full SD-to-DDR payload copy, BBL entry, stable `Linux version 5.1.0-rc7` output, and `smp: Brought up 1 node, 2 CPUs`. The run stopped after `RPC: Registered tcp NFSv4.1 backchannel transport module.` with no `/ #` prompt, and slow input did not echo. The next image-only test should preserve the keep-divisor bootargs and add `keep_bootcon loglevel=8 initcall_debug` before changing hardware.

SBI trace discriminator: use `scripts/p3_make_build67_sbi_trace_image.sh` to generate `build/huaprop3/sd_images/huaprop3_linux_xsbench_2x1_sbi_trace.img` without rerunning Vivado. The first generated image SHA256 is `7bb6ed65ff3bf7e6b1baf3dc067fd32c52faa758a0023a3440e6550f44694543`; the DTB SHA256 is `10ad47f559b12a12ff36bed2c53c45f45152a18700b2da430a83f263fc9395fb`, and the BBL binary SHA256 is `92f5319f5958560c8826799a1a4cb61fea1c2f59af505108e0949bb96f80ffc7`. This image keeps `earlycon=uart8250,mmio,0xfff0c2c000 console=ttyS0,115200n8 rdinit=/bin/sh init=/bin/sh`, adds `keep_bootcon loglevel=8 ignore_loglevel initcall_debug`, and embeds `B67S` prints in BBL's Linux-entry path plus `mcall_set_timer()` and `send_ipi_many()`. In hardware logs, repeating `B67S ipi_wait` points at SBI remote-fence/IPI handling or CLINT MSIP/HLS state; clean SBI progress followed by no console output points back at console handoff/output loss; a change from hart1 boot CPU to hart0 plus farther boot progress points at BBL/Linux boot-hart lottery ordering.

SBI trace hardware result: writing `huaprop3_linux_xsbench_2x1_sbi_trace.img` to `/dev/sdc` and reprogramming the unchanged Build 67 PDI/LTX moved the observed software path past the previous RPC/NFS line. UART log `~/p3_uart_logs/ttyUSB0_20260610_210818_sbi_trace_continue.log` printed `B67S linux_entry_ready hart=0`, `B67S linux_enter hart=0`, `Linux version 5.1.0-rc7`, `smp: Brought up 1 node, 2 CPUs`, `initcall init_sunrpc+0x0/0x82 returned 0`, `initcall populate_rootfs+0x0/0xaa returned 0`, `serial8250_init`, `fff0c2c000.uart: ttyS0 at MMIO 0xfff0c2c000`, and `/bin/sh: can't access tty; job control turned off`. The repeated `B67S ipi_enter ... event=2` records are BBL's `SBI_REMOTE_FENCE_I` handling (`IPI_FENCE_I=0x2`), and this run did not print long `B67S ipi_wait` records, so the trace does not currently show a hard remote-fence wait deadlock. Slow `echo P3_B67_SBI_TRACE_OK` and `uname -a` input did not echo in the same capture; use this result to retire the RPC/NFS stop hypothesis, then switch to a lower-noise shell-validation image before judging UART input or XSBench.

Clean-delay image: use `scripts/p3_make_build67_clean_delay_image.sh` to generate `build/huaprop3/sd_images/huaprop3_linux_xsbench_2x1_clean_delay.img` without rerunning Vivado. This image keeps the clean keep-divisor bootargs and the 2-hart DTB, retains only the small nonzero-hart delay in BBL `boot_other_hart()`, and removes `B67S` tracing, `keep_bootcon`, `ignore_loglevel`, and `initcall_debug`. The first generated image SHA256 is `7936040478b26de64c54b6104469bb35bd5276b46d041be3ce3f17a4abce0241`; the DTB SHA256 is `179005957261afe3ebbc37808bb4035b039612f0dd9fb2227b1161303fe76982`, and the BBL binary SHA256 is `189e0204bdae33192bdef48f8c8c93b03d76f5029b05ee85453ade181cf4611f`. Hardware-test this image before changing RTL: success would implicate BBL/Linux boot-hart ordering and trace/console noise rather than the CLINT/MSIP wait path; failure would mean the SBI trace run's visibility came from `keep_bootcon/initcall_debug` or the remaining console handoff path still needs isolation.
...
### P3 Build 61 SPI SD Pad-Level Debug

Build 61 follows the Build 60 hardware result. A full raw-CSV check showed the OpenCores `send_cmd` block was not idle: response-wait samples included `tx_count=62/63` and repeated `0xff` transmit bytes. Therefore Build 61 keeps the AXI16550 UART path, reference-pin SPI SD wrapper, bootrom stimulus, and four compact BD ILAs unchanged, but adds `P3_SPI_SD_PAD_DEBUG` to expose the actual first CS#-low MOSI bitstream.

```
vivado -mode batch -source scripts/p3_build61_spi_sd_pad_debug.tcl -tclargs -jobs 1
vivado -mode batch -source scripts/p3_build61_spi_sd_pad_debug.tcl -tclargs -skip_create -skip_prepare -reuse_synth -jobs 1
vivado -mode batch -source scripts/p3_program_pdi.tcl -tclargs huaprop3_build61_spi_sd_pad_debug/debug_build/p3_top_build61_spi_sd_pad_debug.pdi
vivado -mode batch -source scripts/p3_ila_capture_build61_spi_sd_pad_debug.tcl
python3 scripts/p3_decode_build61_ila_csv.py huaprop3_build61_spi_sd_pad_debug/debug_build
```
