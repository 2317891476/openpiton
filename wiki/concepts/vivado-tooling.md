...
BBL marker hardware result: writing `huaprop3_linux_xsbench_2x1_bbl_markers.img` to `/dev/sdc` and reprogramming the unchanged Build 67 PDI/LTX proved that BBL reaches the Linux handoff. UART log `~/p3_uart_logs/ttyUSB0_20260609_093656_b67_bbl_markers.log` printed `B67M boot_loader dtb_in=0x0000000080006708 dtb_out=0x0000000081000000 disabled=0x0000000000000000` and `B67M mret hart_arg=0x0000000000000000 entry=0x0000000080200000 dtb=0x0000000081000000`, then emitted the same high-bit/garbled stream with no `Linux version`. Keep using the same Build 67 PDI/LTX for the next discriminator; the issue is now after BBL `mret`, in Linux entry, kernel payload compatibility, early trap handling, or early console setup.
