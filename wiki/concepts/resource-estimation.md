...
|------|----------|-------|----------|-----------|------------|------|-----|-------|
| 2026-06-13 | P3 / VP1902 | Build 71 8x8 UART/SD source-closure diagnostic synthesis | 4,818,444 raw LUT primitives | 2,547,403 raw FD primitives | 4,838 raw synth | 128 | formal report pending | Main `synth_design` completed with 0 errors and 0 critical warnings after 1:32:22 elapsed and about 35.9 GiB peak memory. These are synthesis-log primitive counts, not final CLB utilization; replace them with `p3_top_utilization_synth.rpt` values when that report is generated. Netlist translation also warns that `WEBWE[8]` is dropped when cache `RAMB36E5` instances are replaced by `RAMB36E5_INT`, which is a functional RAM-mapping risk pending implementation and board validation. |
