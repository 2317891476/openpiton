...
|------|----------|-------|----------|-----------|------------|------|-----|-------|
| 2026-06-12 | P3 / VP1902 | Build 68 8x8 OpenSBI/Linux setup | TBD | TBD | TBD | TBD | TBD | 64-core target wrapper and OpenSBI bundle bootrom flow are created, but main 64-core top synthesis/place/route has not completed. The recursive-archive remote run passed bootrom rebuild, 8x8 PyHP generation, self-contained BD/project creation, all four BD ILA OOC runs, `uart_16550`, `clk_wizard`, `proc_sys_reset`, and `axi_noc` OOC synthesis, with zero live `$PPRDIR/../piton/...` XPR paths. Main `synth_1` then failed in RTL elaboration because generated `tile.tmp.v` connected `ariane_verilog_wrap.p3_ariane_debug_bus` but the wrapper did not declare that port. BRAM, routing, congestion, and timing remain unevaluated. |
