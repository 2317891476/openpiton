...
|------|----------|-------|----------|-----------|------------|------|-----|-------|
| 2026-06-11 | P3 / VP1902 | Build 68 8x8 OpenSBI/Linux setup | TBD | TBD | TBD | TBD | TBD | 64-core target wrapper and OpenSBI bundle bootrom flow are created, but main 64-core top synthesis/place/route has not completed. The license-enabled remote run passed bootrom rebuild, 8x8 PyHP generation, BD creation/output products, and all four BD ILA OOC synthesis runs with 0 errors and 0 critical warnings. It then stopped before `synth_1` because the remote archive missed recursive Ariane submodules, leaving live `$PPRDIR/../piton/...` include paths in the XPR. BRAM, routing, congestion, and timing remain unevaluated. |
