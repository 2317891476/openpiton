...
|------|----------|-------|----------|-----------|------------|------|-----|-------|
| 2026-06-11 | P3 / VP1902 | Build 68 8x8 OpenSBI/Linux setup | TBD | TBD | TBD | TBD | TBD | 64-core target wrapper and OpenSBI bundle bootrom flow are created, but Vivado synthesis/place/route has not been run yet. The first remote attempt reached Vivado 2024.2.2 but stopped in bootrom rebuild because `riscv64-unknown-elf-gcc` was missing on offline Ubuntu, so there is no 8x8 resource datapoint yet. BRAM and routing remain the primary implementation risks after the toolchain blocker is cleared. |
