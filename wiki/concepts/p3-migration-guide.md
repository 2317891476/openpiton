...
As of the paused 2026-06-11 SD-image campaign, the Build 67 evidence should be read as a progression, not as independent guesses. The keep-divisor image removed the explicit early-console baud rewrite and proved stable Linux ASCII plus 2-hart bring-up. The SBI trace image proved the old SunRPC line was only the last visible line without `keep_bootcon`: `init_sunrpc` returned, `populate_rootfs` completed, `serial8250_init` registered the AXI16550, and `/bin/sh` launched. The clean-delay image then proved the small nonzero-hart delay alone was not the fix. The low-noise image retained `keep_bootcon` and `initcall_debug` without high-volume firmware prints and exposed the true post-shell symptom: repeated CPU1 RCU/forward-progress stalls and no successful throttled UART command. The IRQ-chain image showed that the remote-fence burst during executable-page mapping completed before the persistent stall, then highlighted an inconsistent hart1 timer pattern. The MTIP-state image proved the sampled hart1 trap was machine timer interrupt cause 7 and that the normal `csrc mie, MTIE` operation can clear bit 7 on at least the recorded entry. The safe MTIP image reduced diagnostic perturbation, but the tested pre-format-fix copy then produced an illegal-instruction panic on CPU1 at `sepc=0xffffffe0006e4004`; local disassembly shows that address contains a valid instruction sequence in the kernel image, so this symptom should be treated as possible stale or wrong instruction fetch until disproven.

The strongest current exclusions are therefore: not the SD block transport, not DDR payload copy, not BBL `mret`, not Linux early console entry, not a hard SunRPC stop, and not a simple PLIC source-count mismatch after the DTB fix to `riscv,ndev = <2>`. The remaining root-cause branches are CPU1-specific forward progress under SMP: timer/IPI delivery to tile1, BBL's M-mode redirect to supervisor STIP/SSIP, Linux's scheduler/RCU handling after those interrupts, or instruction/cache coherence around the remote-fence and executable-page path. Do not claim a final named RTL bug from the current logs. The regenerated fixed-format safe MTIP image has SHA256 `c908aa0ff09800349981125d1a8979eca498ed9f756265e7139f680b0daae21d` and fixes the riscv-pk `%u` trace-format bug, but it has not been written/tested on hardware yet because the remote reader was empty when the campaign paused.
...
The supplied `riscv64-linux-64core-src-20260610.tar.gz` omits multiple Linux 6.6 build inputs, including `arch/riscv/kernel/vmlinux.lds.S`, `include/asm-generic/vmlinux.lds.h`, and the linker scripts for many other architectures. `scripts/p3_prepare_64core_opensbi_image.sh` therefore verifies an official `linux-6.6.tar.xz` baseline with SHA256 `d926a06c63dd8ac7df3f86ee1ffc2ce2a3b81a2d168484e76b5b389aba8e56d0` and restores only files missing from the packaged tree; existing package files and the custom SBI batch changes are never overwritten. The default baseline path is `build/p3_64core/linux-6.6.tar.xz`, overridable with `P3_64CORE_LINUX_BASE_ARCHIVE`. The script also validates the extracted OpenSBI, Linux, and config inputs before building, and passes `--file` to Linux `scripts/config` so `CONFIG_NR_CPUS=64`, `CONFIG_SMP=y`, and `CONFIG_RISCV_SBI=y` are applied to the kernel tree rather than an accidental repository-root `.config`.
