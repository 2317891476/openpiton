...
After the XSBench ext2-image validation, Build 66 became the canonical local baseline. The published artifacts are kept under `huaprop3_build66_baseline/debug_build/`, with future clean Vivado rebuilds directed to `D:/p3_baseline_build66` by `scripts/p3_build66_normal_spi_sd_boot.tcl`. Historical debug projects and old numbered Vivado workspaces were moved to archive directories outside the repository root; `huaprop3onecore/` remains available as the board-level reference project.
