...
For that isolation step, `build/huaprop3/sd_images/huaprop3_linux_shell.img` patches the BBL embedded bootargs to `rdinit=/bin/sh init=/bin/sh`. This image should bypass the normal init scripts and verify whether the same kernel/initramfs can provide an interactive shell. If it reaches `/bin/sh`, the remaining work is rootfs init policy (for example replacing a blocking random-seed step or changing bootargs such as `random.trust_cpu=on`); if it still does not, the focus shifts to userspace console input, `/bin/sh` availability, or initramfs content.
