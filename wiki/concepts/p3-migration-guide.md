...
The `bbl_markers` hardware discriminator was tested on 2026-06-09 with the same Build 67 PDI/LTX. UART log `ttyUSB0_20260609_093656_b67_bbl_markers.log` again cleared the bootrom, SPI-SD, GPT, DDR payload-copy, and BBL-entry path, then printed `B67M boot_loader dtb_in=0x0000000080006708 dtb_out=0x0000000081000000 disabled=0x0000000000000000` and `B67M mret hart_arg=0x0000000000000000 entry=0x0000000080200000 dtb=0x0000000081000000`. This proves BBL selects hart0, places the final DTB at `0x81000000`, and executes `mret` into the Linux payload at `0x80200000`. The repeated high-bit/garbled bytes occur after this handoff, before any visible `Linux version` banner, so the next convergence path should stay software-only: compare Build 66 and Build 67 payload formats, verify Linux entry assumptions, or insert a tiny post-`mret` UART marker payload before spending another synthesis run.
