...
3. **Transition to SiFive UART (Build 40) & Instruction Fetch Hang Analysis**:
   ...
   - *Build 42-B implementation*: add a gated `P3_BUILD42B_BRAM_STACK` AXI interposer for the normal C bootrom. The interposer captures the translated stack window `0x03ff0000..0x04000000` on the 512-bit OpenPiton memory AXI port, equivalent to CPU `0x83ff0000..0x84000000`, and leaves CPU `0x80000000` routed to DDR for the SD/BBL load buffer. This makes Build 42-B a controlled stack-path experiment, not a replacement for the Build 42-A UART bridge investigation.
