...
| Current scaling candidate | Build 68 8x8 / 64-core OpenSBI/Linux completed VP1902 implementation, PDI/LTX generation, P3 Pro programming, debug-hub refresh, bundle-bootrom UART, and final source-rebuilt Linux 6.6/OpenSBI/64-hart SD image generation. Build 69 programmed successfully on 2026-06-13 with `DONE bit: HIGH` and four ILAs but emitted 0 UART bytes. Its ILA capture proves heartbeat/reset release, core/NOC activity, successful DDR reads, and then an SPI-SD transaction-manager read error with no nonzero response data. Build 70 is now the automatic active build: instrument bootrom UART writes through AXI16550 TX directly and retain a narrow first-SD-request/error trace instead of repeating the broad Build 69 probes. |
