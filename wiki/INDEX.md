...
| Current scaling candidate | Build 68 8x8 / 64-core OpenSBI/Linux completed VP1902 implementation, PDI/LTX generation, P3 Pro programming, debug-hub refresh, bundle-bootrom UART, and final source-rebuilt Linux 6.6/OpenSBI/64-hart SD image generation. Build 69 programmed successfully on 2026-06-13 with `DONE bit: HIGH` and four ILAs but emitted 0 UART bytes; its ILA capture proved reset release, core/NOC activity, successful DDR reads, then an SPI-SD transaction-manager read error. Build 70 is the active automatic successor. Its four diagnostic ILA OOC runs and self-contained 8x8/64-tile validation passed, and the main remote `synth_1` run started at 13:37 CST. It directly retains bootrom/core UART stores, AXI16550 handshakes/responses, TX transitions, compact SD progress/error flags, and the targeted SD debug CDC correction. |
