...
| Current scaling candidate | **2-core (2x1) bring-up in progress.** Build 73 plus the no-CSR probe image passed bootrom, OpenSBI v1.8, and Linux early console. The controlled marker split (II[SHDMW with CSR 0x701 versus II[SHDMWRTEPB without it) proves that Ariane's D-cache-disable mode blocks OpenSBI. Historical 64-hart Build 68 dbg23 had the same failure, so P3 firmware now keeps L1 D-cache enabled at every hart count. A clean non-probe image still needs SD write/readback and board validation; Linux SMP completion and a 2-core shell remain unproven. **See [HANDOFF_codex_2core_opensbi.md](HANDOFF_codex_2core_opensbi.md) for the full handoff.** |
