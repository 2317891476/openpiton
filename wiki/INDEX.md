...
| Current scaling candidate | Build 68 8x8 / 64-core OpenSBI/Linux completed VP1902 implementation, PDI/LTX generation, P3 Pro programming, debug-hub refresh, bundle-bootrom UART, and final source-rebuilt Linux 6.6/OpenSBI/64-hart SD image generation. Build 69 completed 8x8 implementation and programmed successfully on 2026-06-13 with `DONE bit: HIGH`, debug hub `0x3ffc0000000`, and four ILAs, but both UART ports stayed at 0 bytes and the no-stack `B69 ASM` marker did not appear. The active gate is Build 69 ILA capture followed by an automatic narrower UART-first build; DDR, SD/GPT/`P3OS`, OpenSBI, and Linux are downstream of the current first failing layer. |
