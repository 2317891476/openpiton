...
| Current scaling candidate | Build 68 8x8 / 64-core OpenSBI/Linux completed VP1902 implementation, PDI/LTX generation, P3 Pro programming, debug-hub refresh, bundle-bootrom UART, and final source-rebuilt Linux 6.6/OpenSBI/64-hart SD image generation. Build 69 programmed successfully on 2026-06-13 with `DONE bit: HIGH` and four ILAs but emitted 0 UART bytes; its ILA capture proved reset release, core/NOC activity, successful DDR reads, then an SPI-SD transaction-manager read error. Build 70 retained direct UART/SD evidence but failed main synthesis because the new `piton_spi_sd_top` instance was present while its source was omitted from `git archive HEAD`. Build 71 is the active automatic successor; the full 32-job offline-Ubuntu flow completed successfully through PDI/LTX publication. All 6,939,940 routable nets are fully routed with 0 routing errors; formal timing is `WNS=5.094 ns`, `TNS=0`, `WHS=0.010 ns`, `THS=0`, `WPWS=0.063 ns`, and `TPWS=0`. Published artifacts are a 238,485,984-byte PDI with SHA256 `3c8db9c40f683988b0bb7801c61401768a641d9acdbb0db1ce220291f962cf19` and a 62,285-byte validated LTX with SHA256 `9c99e06e9ade212d26a6f020c13801688134d46f449378452226815e94ef9b7a`; local hashes match. Two Windows-Vivado programming attempts stalled for 32:39 and 34:58 without reaching `DONE bit: HIGH`, including one after restarting hw_server. This remains an XVC/programming gate; the next retry moves Vivado Lab and the artifacts to the board-side Ubuntu host before any Build 72 decision. |
