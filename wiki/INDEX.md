...
| Current scaling candidate | **2-core (2x1) bring-up in progress.** 64-core (8x8) reaches an interactive shell (`nproc=64`) but hangs under load. 2-core as a controlled experiment: bootrom runs fully + jumps to OpenSBI, but **OpenSBI prints no banner** (hangs in cold boot). All 64-core OpenSBI fixes verified present in 2-core source (static audit done). ILA board evidence (2026-07-13): CPU internally hung after accessing `console_dev`, no UART output. The image had carried an unverified fw (`dbg28b_noprobe`); dbg21 fw is the candidate fix but is at-risk (console-path evidence). Exact failing step pending a probe-fw boot (`I[SHDMWRTEPB` markers), which needs a physical SD card swap. **See [HANDOFF_codex_2core_opensbi.md](HANDOFF_codex_2core_opensbi.md) for the full handoff.** |
