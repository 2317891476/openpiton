# 移植与多核扩展技术周报：从单核基准到64核扩展与硬件调试阶段性进展 (6月4日 - 6月15日)

> 2026-07-02 证据审计说明：本周报保留 6月4日 到 6月15日 当时的阶段性叙述，
> 但不能作为当前 64 核根因结论直接使用。后续 `coh_ipi64.c`
> progress-only 8x8 仿真三连 PASS，已经把旧的 L1.5 monitor fail 从“已复现根因”
> 降级为“未确认旧线索”。当前调试应优先按板上分层证据链推进：PDI DONE、UART
> TX、DDR、SPI-SD/GPT/`P3OS`、OpenSBI entry、Linux SMP progress。

本报告汇总了从 **6月4日（单核验证完成）至 6月15日（本周一）** 期间，在 **HuaPro P3 (Versal VP1902)** 评估板上移植 **OpenPiton + Ariane (CVA6)** 平台的调试进展。主要工作涵盖了 Build 66 单核基准自包含编译的闭合与验证、Build 67 双核多核收尾验证、64核大系统实现与低级诊断（Build 68-69）、编译源文件闭合（Build 70-71），以及在硬件调试阶段对 JTAG-XVC 链路高延迟下载瓶颈的突破。

---

## 一、 核心调试里程碑

在这段跨度期间，我们围绕单核基准固化、64核系统的物理实现、总线与外设诊断，以及调试流程优化，完成了以下关键工作：

```mermaid
gantt
    title P3 多核扩展与调试进度 (6月4日 - 6月15日)
    dateFormat  YYYY-MM-DD
    section 单核基准与自包含机制
    Build 66: 单核基准上板验证与自包含Include修复 :active, 2026-06-04, 2026-06-05
    section 双核收尾与64核启动
...
### 0. Build 66 (单核正常引导基准线与自包含快照闭合)
* **遇到问题**：
  * **Include 缺失导致编译失败**：在首次尝试使用源码自包含快照机制（`P3_SELF_CONTAINED_SOURCES=1`）从 `p3b66/source_snapshot/` 重新编译 Build 66 时，由于脚本设计缺陷，仅拷贝了 include 文件夹的第一层级，导致 nested 包含的 Ariane/CVA6 头文件（如 `register_interface/assign.svh` 等）缺失，造成 RTL 综合阶段直接报 elaboration 错误。
  * **串口输入溢出导致控制台无响应**：在早期成功引导 Linux 进入交互式 shell 后，向 `/dev/ttyUSB0` 批量粘贴或快速输入测试命令时，频繁触发 AXI16550 UART 的 `ttyS0 input overrun(s)` 警告，进而导致字符丢失或输入死锁。
* **排查与解决过程**：
  * **自包含快照修复**：更新了 `scripts/p3_create_bd.tcl` 中的复制逻辑，使其能够递归地、按相对路径完整地将所有头文件子目录拷贝至 snapshot 目录，通过了无外部 live 目录引用的严格时序闭合编译。
  * **慢速输入脚本限制**：定位到 AXI16550 对大吞吐量无握手输入存在物理缓冲限制。我们在板端及本地部署了带有时间间隔（每个字符延时约 0.2 秒）的 Python 写脚本，限制串口写入速率，成功打通了稳定的 shell 交互。
  * **最终结论**：Build 66 实现了单核 Linux 稳定引导、DDR4 访问及 SD 卡 ext2 挂载。同时，其确立的 **自包含快照编译机制** 成为后续所有 64 核扩展工程的强制性前置规范。
...
* **排查与解决过程**：
  ...
  * **脚本修正**：重构了 `p3_rebuild_build68_8x8_opensbi_linux.sh`，生成一个 temporary 最小化 DTS 以自动编译生成 baremetal `bootrom.sv` 以满足例化要求。
...
2. **大容量 PDI 烧录看门狗策略**：
   * Versal 级别的 238MB PDI 下载文件庞大，而 Tailscale 连接 of XVC 存在单次 transaction 约 40ms 的固有网络往返高延时。
